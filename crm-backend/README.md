...
Для запуска сервера перейдите в папку с репозиторием, затем в каталог `crm-backend` в папке репозитория, и выполните команду `npm start`. Для остановки нажмите сочетание клавиш CTRL+C.

Для первоначального наполнения базы данных клиентов (файла  `crm-backend/storage/clients.json`) вместо запуска сервера можно выполнить команду `npm generateDB`. Все имеющиеся данные клиентов будут при этом удалены. Параметры генерации базы клиентов можно посмотреть и настроить в файле `crm-backend/.env`.
